...
STOPWORDS = set(["the", "a", "of", "on", "and", "to", "in"])
...
def normalize_logs(logs):
    """
    Normalize log messages by removing IP addresses and replacing numbers.
    """
    for log_category in logs.values():  # Iterate over both 'normal' and 'anomaly' logs
        for log in log_category:
            log["message"] = re.sub(r'\d+\.\d+\.\d+\.\d+', '<IP>', log["message"])  # Mask IP addresses
            log["message"] = re.sub(r'\d+', '<NUM>', log["message"])  # Mask numbers
    return logs

def group_events(logs):
    """
    Group similar log events by replacing specific words with placeholders.
    """
    for log_category in logs.values():  # Iterate over both 'normal' and 'anomaly' logs
        for log in log_category:
            log["message"] = re.sub(r'\bnode-\d+\b', '<NODE>', log["message"])  # Replace node identifiers
            log["message"] = re.sub(r'pool \d+', 'pool <POOL>', log["message"])  # Replace pool numbers
    return logs
...
def augment_anomalies(logs):
    augmented_logs = logs.copy()
    for log in logs:
        if log["label"] == 1:  # Only augment anomalies
...
def tokenize(logs):
    tokenized_logs = []
    for log in logs:
        tokens = [log["level"].lower(), log["component"].lower(), log["node"].lower()] + tokenizer.tokenize(log["message"].lower())
...
def encode_logs(tokenized_logs, vocab, max_seq_len=50):
    for log in tokenized_logs:
        log["sequence"] = [vocab.get(word, vocab["<UNK>"]) for word in log["tokens"][:max_seq_len]]
        log["sequence"] += [0] * (max_seq_len - len(log["sequence"]))
    return tokenized_logs
...
def preprocess_logs():
    ...
    logs = group_events(logs)

    print("Balancing dataset...")
    balanced_logs = balance_data(logs)

    print("Augmenting anomalies...")
    balanced_logs = augment_anomalies(balanced_logs)
